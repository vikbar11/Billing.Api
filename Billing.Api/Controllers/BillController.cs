...
namespace Billing.Api.Controllers
{
    ...
    public class BillController : ControllerBase
    {
        ...
        public async Task<ActionResult<BillDetailInvDTO>> GetBill(int id)
        {
          if (_context.Bills == null)
          {
            ...
            var bill = await _context.Bills
                ...
                .ThenInclude(billd => billd.IdProductNavigation)
                .Where(billd => billd.IdBill == id)
                .FirstAsync();

            List<BillDetailInvDTO> details = new List<BillDetailInvDTO>();

            bill.BillDetails.ToList().ForEach(async billd =>
            {
                BillDetailInvDTO detail = new BillDetailInvDTO();
                detail.ProductNameDTO = billd.IdProductNavigation.ProductName;
                detail.PriceDTO = billd.IdProductNavigation.Price;
                detail.QuantityDTO = billd.Quantity;
                details.Add(detail);
            });
        ...
        public async Task<ActionResult<Bill>> PostBill(BillDTO billDTO)
        {
            ...
            Bill bill = new Bill();
            bill.BillDate = billDTO.BillDateDTO;
            bill.IdCustomer = billDTO.IdCustomerDTO;
